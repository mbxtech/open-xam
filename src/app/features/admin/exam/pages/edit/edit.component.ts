...
import {
  ToggleGroup,
  ToggleOption,
} from "../../../../../shared/forms/toggle-group/toggle-group";
...
import {
  StatusType,
  statusTypeSelectOptions,
} from "../../../../../shared/model/status-typ.enum";
...
import {
  TEXT_REGEX,
  TEXT_VALIDATORS,
} from "../../../../../shared/forms/validation/validators-sets";
...
export class EditComponent implements OnDestroy {
  ...
  protected invalidElementsSet: WritableSignal<Map<number, HTMLElement>> =
    signal(new Map());
  ...
  protected readonly toggleOptions: ToggleOption[] =
    statusTypeSelectOptions().map((opt) => {
      return {
        ...opt,
        disable: false,
      };
  ...
  constructor() {
    ...
    this.pageTitle.set(
      $localize`:@@ox.exam.edit.page.create.title:Create new exam`,
    );
    this.pageSubTitle.set(
      $localize`:@@ox.exam.edit.page.create.subTitle:Create a new exam`,
    );
    ...
    if (this._route.snapshot.params) {
      ...
      if (id > 0) {
        this._subscriptions$.add(
          this._service.getExamById(id).subscribe((ex) => {
            if (ex?.id) {
      ...
      if (importID) {
        ...
        this._subscriptions$.add(
          this._importCacheService
            .loadInvalidFile(Number(importID))
            .subscribe((result) => {
    ...
    this._subscriptions$.add(
      this._service.errors$.subscribe((errors) => {
        if (errors.length) {
          errors.forEach((err) => {
            this._toastService.addErrorToast(
              $localize`:@@ox.errors.exam.edit:Error during exam operation`,
              err,
            );
          });
        }
  ...
  private _setInitialValues(ex: IExam): void {
    ...
    this.pageTitle.set(
      $localize`:@@ox.exam.edit.page.title:Editing exam: ${ex.name}`,
    );
    this.pageSubTitle.set(
      $localize`:@@ox.exam.edit.page.subTitle:Editing the existing exam`,
    );
  }
  ...
  public submit(): void {
    if (!this._formGroup.valid) {
      ...
      this._addToastMessage(
        $localize`:@@ox.exam.edit.errors.form.not.valid:Form is not valid, please check all fields.`,
        false,
      );
      return;
    }
        ...
        this._service.createExam(new Exam(data)).subscribe((result) => {
          if (result?.id) {
            if (this.importID()) {
              this._subscriptions$.add(
                this._importCacheService
                  .deleteInvalidFile(this.importID()!)
                  .subscribe({
                    next: () => {
                      this._logger.logInfo(
                        `Invalid file with id: ${this.importID()} deleted successfully`,
                      );
                    },
                    error: (err) => {
                      this._logger.logError(
                        `Error during deleting invalid file with id: ${this.importID()}`,
                        err,
                      );
                    },
                  }),
            ...
            this._addToastMessage(
              $localize`:@@ox.exam.edit.update.success:Successfully created exam ${result.name} with id: ${result?.id}`,
              true,
            );
            this._formGroup.reset();
            this._router
  ...
  private _readAllInvalidFields(): void {
    const collection: HTMLCollectionOf<Element> =
      document.getElementsByTagName("ox-basic-input");
