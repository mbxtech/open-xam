...
export class AbstractEdit {
  ...
  protected directActionDialogState: DirectActionDialogState =
  ...
  protected openDirectActionDialog(
    ...
    this.directActionDialogState = {
      ...
      submitLabel:
        config.submitLabel ?? directActionDialogInitState.submitLabel,
      actionBtnLabel:
        config.actionBtnLabel ?? directActionDialogInitState.actionBtnLabel,
      onAbort,
      onSubmit,
      open: true,
      onAction,
  ...
  protected addAnswerFormGroup(
    value?: IAnswer,
    assignedOptionId?: number,
  ): FormGroup {
    ...
    const fg = this._formBuilder.group({
      ...
      assignedOptionId: new FormControl(
        value?.assignedOptionId ?? assignedOptionId ?? null,
      ),
  ...
  protected addQuestion(question?: IQuestion) {
    const fg = this._formBuilder.group({
      ...
      examId: new FormControl<number | null>(question?.examId ?? null),
    });
    ...
    if (question?.options?.length) {
      question.options.forEach((option) => {
        (fg.get("options") as FormArray).push(
          this.addAssignmentOptions(option.id, option),
        );
      });
    }
  ...
  protected _getGrpByInternalId(
    array: FormArray,
    internalId: string,
  ): FormGroup | null {
    const grp = array.controls.find(
      (grp) => grp.get("internalId")?.value === internalId,
    );
    if (!grp) {
